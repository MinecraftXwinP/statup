...
`
	...
	assert.Equal(t, int64(1530841150), migrations[0].version)
	assert.Equal(
		t,
		"ALTER TABLE core ADD COLUMN use_cdn BOOL NOT NULL DEFAULT '0';",
		strings.TrimSpace(migrations[0].statements),
	)
	assert.Equal(t, int64(1532068515), migrations[1].version)
	assert.Equal(
		t,
		`ALTER TABLE services ALTER COLUMN order_id integer DEFAULT 0;
ALTER TABLE services ADD COLUMN timeout integer DEFAULT 30;`,
	...
	assert.Equal(t, int64(1532068518), migrations[2].version)
